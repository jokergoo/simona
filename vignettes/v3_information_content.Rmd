...
where $p(x)$ is the probability of term $x$ under a certain measure. If a term
has smaller probability, it means it is more specific, thus the form
$-\log(p(x))$ is called the "information content". Basically the more to the leaves
in the DAG, the higher information content a term has.
...
here I also set the gene annotations because some IC methods depend on 
the number of annotated genes on terms.
...
And the function `term_IC()` calculates information contents of terms in the DAG with a specific 
method.
...
$$ G_x = G'_x \bigcup \left( \bigcup_{z \in \mathcal{C}(x)} G_z \right) $$
...
In some implementations, number of annotated items on a term is calculated by
simply adding such numbers from all its child terms, as shown in the following
equation:

$$ f(x) = \left\{ \begin{array}{lr}
\beta(x) & \text{if } \textit{x} \text{ is a leaf} \\
\beta(x) + \sum\limits_{z \in \mathcal{C}} \beta(z) & \text{otherwise}
\end{array} \right. $$


where $\beta(x)$ is the number of items annotated to term $x$. It should be
noted that this way over-estimates the number of annotated items because an
item can be annotated to multiple terms. For example, a gene can be annotated
to multiple GO terms (you can check the object `org.Hs.eg.db::org.Hs.egGO`.
And even for the same term, it may have duplicated annotated genes with
multipel evidence codes (e.g. the gene with Entrez ID `"994"` is annotated to
a GO term `"GO:0000086"` twice with evidence code `"IBA"` and `"TAS"`.
...
annotations. We can specify an additional parameter for `IC_annotation`:
...
It shows anyway the trend from the two methods is very linear.
...
external data and people argue the IC should be an intrinsic attribute and
only depends on the DAG structure itself. All other IC methods introduced in
later sections only reply on the DAG structure itself.
...
To understand this method, let's think the DAG relates a kind of information
transmission from the root to the inner of the DAG, where the information is
100% on root. Then, in a certain step in the DAG, an intermediate term $x$ has
two parents $t_1$ and $t_2$, also let's assume $t_1$ has $k_1$ children and
$t_2$ has $k_2$ children, next we assume a parent transmits information
equally to all its children, then $t_1$ only transmits $1/k_1$ and $t_2$ only
transmits $1/k_2$ of its content to term $x$, on which we can also say, the
...
\prod\limits_{t \in \mathcal{P}(x)} \frac{p(t)}{|\mathcal{C}(t)|} & \text{otherwise}
\end{array} \right. $$
...
It measures the number of ways from a term to reach leaf terms. E.g. in the
following DAG:
...
The number of ways for term $x$ to reach leaf terms can be obtained in a
recursively way:

$$ f(x) = \left\{ \begin{array}{cl}
1 & \text{if } \textit{x} \text{ is a leaf} \\
\sum\limits_{t \in \mathcal{C}(x)} f(t) & \text{otherwise}
\end{array} \right. $$
...
It measures average contribution of term $x$ on leaf terms. First denote
$\zeta_x$ as the number of leaf terms that can be reached from term $x$ (or in
other words, $x$'s offspring terms that are leaves). Since all $x$'s ancestors
can also reach $x$'s leaves, the contribution of $x$ on leaf terms is scaled
by the number of its ancestors. The final information content is normalized by
the total number of leaves in the DAG. 
...
$$ r_2 = 1 - \frac{\log \left(1 + \sum_{t \in \mathcal{D}_x} \frac{1}{\delta(t)} \right)}{\log(\lambda_\mathrm{node})} $$
...
1 & \text{if } \textit{t = x} \\
\max\limits_{t' \in \mathcal{C}(t)}{w_e * S_x(t')} & \text{otherwise}
\end{array} \right. $$
...
0 & \text{if } \textit{t = x} \\
\min\limits_{t' \in \mathcal{C}(t)}{\frac{1}{w_e} * T_x(t')} & \text{otherwise}
\end{array} \right. $$
...
Normally _IC_Wang_2007_ is only used ...
...
And the heatmap of the correlation. Using `use = "pairwise.complete.obs"` because
there might be `NA` values for ...
