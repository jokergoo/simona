...
and $\mathbf{v_q}$. Taking $v_p$ as an example, it is $\mathbf{v_p} = (w_1, ..., w_n)$ where $n$
...
$$ \mathrm{Sim}(a, b) = \frac{ \mathbf{v_p} \cdot \mathbf{v_q} }{\left \| \mathbf{v_p} \right \| \cdot  \left \| \mathbf{v_q} \right \|} $$

It can also be written as:

$$ \mathrm{Sim}(a, b) = \frac{\sum\limits_{x \in \mathcal{A}_p \cap \mathcal{A}_q}\mathrm{IC}(x)^2}{\sqrt{\sum\limits_{x \in \mathcal{A}_p}\mathrm{IC}(x)} \cdot \sqrt{\sum\limits_{x \in \mathcal{A}_p}\mathrm{IC}(x)}} $$

## GroupSim_SimCOT

The semantic similarity is defined as:

$$ \mathrm{Sim}(a, b) = \frac{ \mathbf{v_p} \cdot \mathbf{v_q} }{\left \| \mathbf{v_p} \right \|^2 +  \left \| \mathbf{v_q} \right \|^2 - \mathbf{v_p} \cdot \mathbf{v_q}} $$
